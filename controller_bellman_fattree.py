...
# 
# CONTROLLER FAT-TREE - BELLMAN FORD (DECOUPLED MONITOR FIX)
# Perbaikan Fundamental:
# 1. DECOUPLED MONITOR: Tidak lagi menggunakan Event Handler untuk trigger update.
#    Menggunakan Thread Loop terpisah yang berjalan periodik (5-10 detik).
#    Ini menghilangkan 'Event Storm' yang menyebabkan 'Multiple connections'.
# 2. STRICT SILENCE: Drop paket sampai MST siap.
# 3. TPOOL: Kalkulasi berat tetap di thread terpisah.
...
class BellmanFatTreeController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("BellmanFatTreeController: Siap (Decoupled Monitor Mode).")
        
        # Jalankan Monitor Topologi di thread terpisah sejak awal
    ...
    # --- PERUBAHAN BESAR: EVENT HANDLER KOSONG ---
    # Kita sengaja mengabaikan event link add/delete secara langsung
    # untuk mencegah CPU overload saat ribuan event masuk sekaligus.
    # Pekerjaan diambil alih oleh _monitor_topology.
    ...
    def _monitor_topology(self):
        """
        Loop abadi yang mengecek topologi secara periodik (Polling).
        Jauh lebih ringan daripada Event-Driven untuk skala besar.
        """
        while True:
            # Interval cek: 5 detik (Bisa dinaikkan jika masih berat)
            hub.sleep(5.0)
            self._build_optimal_topology()

    def _build_optimal_topology(self):
        # Ambil data terbaru dari Ryu Topology API
        ...
        # Hitung MST di Thread Terpisah (TPOOL)
        ...
        # Logging Cerdas (Hanya print jika status berubah)
        if len(self.net.nodes) > 0:
    ...
    def _intelligent_flood(self, datapath, in_port, msg):
        # Strict Silence: Jangan ngapa-ngapain kalau MST belum jadi
        parser = datapath.ofproto_parser
        dpid = datapath.id

        # If MST not computed yet, fallback to normal flooding so ARP/LLDP can propagate
        if self.mst is None:
            actions = []
    ...
    def _packet_in_handler(self, ev):
        ...
        if dpid == dst_dpid:
            ...
            try:
                # Bellman-Ford Calculation via TPOOL
